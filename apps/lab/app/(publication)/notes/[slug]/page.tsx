...
export function generateStaticParams() {
  return getPublishedEntries().filter((entry) => entry.kind === "note").map(({ slug }) => ({ slug }));
}

export async function generateMetadata({ params }: Props) {
  const { slug } = await params;
  const entry = getPublishedEntry("note", slug);
  if (!entry) notFound();
...
export default async function NotePage({ params }: Props) {
  ...
  const entry = getPublishedEntry("note", slug);
  if (!entry) notFound();
  return <EditorialArticle entry={entry} afterBody={slug === "security-scope-and-data-flow" ? <ArchitectureExplorer /> : undefined} />;
}
