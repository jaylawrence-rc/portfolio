...
export default async function ProjectPage({ params }: { params: Promise<{slug:string}> }) {
  ...
  return <article className="case-study" style={{"--project-accent":project.accent} as React.CSSProperties}>
    ...
    <div className="article-body reading-rail">
      ...
      <section><p className="eyebrow">Key decisions</p><h2>The choices that made the system clearer.</h2>{project.decisions.map((d,i)=><div className="decision" key={d.title}><span>0{i+1}</span><div><h3>{d.title}</h3><p>{d.body}</p></div></div>)}</section>
    </div>
