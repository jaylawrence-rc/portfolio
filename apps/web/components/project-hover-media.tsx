...
export function ProjectHoverMedia({ project, compact = false }: { project: Project; compact?: boolean }) {
  ...
  return (
    <div className="project-hover-media" data-project-hover-media>
      ...
      <div className={`project-hover-sheet ${detail || project.hoverPage ? "project-hover-sheet--image" : "project-hover-sheet--decision"}`} aria-hidden="true">
        {project.hoverPage ? <ProjectLivePreview key={project.hoverPage.url} {...project.hoverPage} /> : detail ? <><Image src={detail.src} alt="" width={detail.width} height={detail.height} sizes="(max-width: 780px) 75vw, (max-width: 1200px) 50vw, 720px" /><div className="project-hover-sheet-footer"><span>{detail.label}</span><span>Inside the project <ArrowUpRight size={14} /></span></div></> : <>
