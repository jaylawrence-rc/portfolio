...
export function WorkFilter() {
  ...
  return <><div className="filters" aria-label="Filter projects">{filters.map(filter => <button key={filter} aria-pressed={current === filter} onClick={() => select(filter)}>{filter}</button>)}</div><p className="result-count" aria-live="polite">{visible.length} {visible.length === 1 ? "project" : "projects"}</p><div className="project-list">{visible.map((project, index) => <ProjectRow project={project} index={projects.indexOf(project)} key={project.slug} compact={!project.featured}/>)}</div></>;
}
